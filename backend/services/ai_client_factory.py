...
from config import GEMINI_EMBEDDING_MODEL, GEMINI_MODEL
...
def _cached_gemini_embeddings(api_key: str, model_name: str) -> GoogleGenerativeAIEmbeddings:
...
def make_embeddings_for_user(user_id: int, db: Session) -> Embeddings:
    ...
    Vector size must match QDRANT_VECTOR_SIZE and the chosen GEMINI_EMBEDDING_MODEL.
    """
    api_key = _require_gemini_api_key(db, user_id)
    return _cached_gemini_embeddings(api_key, GEMINI_EMBEDDING_MODEL)
