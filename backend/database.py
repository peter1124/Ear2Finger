...
def migrate_db():
    ...
    try:
        ...
        if 'playlists' in table_names:
            with engine.begin() as conn:
                # Remove duplicate playlist names, keeping the lowest id for each name
                conn.execute(
                    text(
                        """
                        DELETE FROM playlists
                        WHERE id NOT IN (
                            SELECT MIN(id) FROM playlists GROUP BY name
                        )
                        """
                    )
                )
                # Create a unique index on playlist names if it does not exist
                conn.execute(
                    text(
                        """
                        CREATE UNIQUE INDEX IF NOT EXISTS ix_playlists_name_unique
                        ON playlists (name)
                        """
                    )
                )
                print("Database migrated: Ensured unique playlist names")
